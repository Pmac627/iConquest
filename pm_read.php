...
$site_area = 'main';
$page_title_name = 'Reading Private Message';
...
if(isset($_COOKIE['ID_i_Conquest']))
{
	...
	else
	{
		...
		$message_stats = mysql_query("SELECT ID_recip, ID_send, read_pm, subject, body, time_sent FROM private_message WHERE ID_message = '$ID_message'") or die(mysql_error());
		while($pm = mysql_fetch_array( $message_stats ))
		{
			$ID_recip = stripslashes($pm['ID_recip']);
			$ID_send = stripslashes($pm['ID_send']);
			$read_pm = stripslashes($pm['read_pm']);
			...
			{
				// Redirect them to the error page
				header("Location: error_page.php?error=45");
			}
			...
			if($read_pm == 0)
			{
				...
				// update to read
				$insert = "UPDATE private_message SET read_pm='" . $new_read_pm . "', read_date='" . $read_date . "' WHERE ID_message='" . $ID_message . "'";
				$add_member = mysql_query($insert);
			}
		}

		//collect the sender's information for display from the users table
		$PM_sender = mysql_query("SELECT username FROM users WHERE ID = '$ID_send'") or die(mysql_error());
		while($send = mysql_fetch_array( $PM_sender ))
		{
			// put $ID_sent to a sender name
			$sender = stripslashes($send['username']);
		}
		...
		<table class='list_central'>
			...
			<tr>
				<td class='button' colspan='4'><input type='hidden' name='sender' value='$sender' />
				<input type='button' onClick="parent.location='pm_send.php?to=<?php echo $sender; ?>'" name='reply' value='Reply' />
				<input type='button' onClick="parent.location='pm_central.php'" name='back' value='Back to PM Central | Inbox' /></td>
			</tr>
		</table>
		</form>
		</td>
