...
$site_area = 'main';
$page_title_name = 'Search for Nation';
...
if(isset($_COOKIE['ID_i_Conquest']))
{
	...
	{
		//otherwise they are shown the nation search page
		...
		<table class='search_display'>
			<tr>
				<th class='form_head' colspan='4'>Nation Search</th>
			</tr>
			<tr>
				...
				<table class='search_box'>
					<tr>
						...
						<td class='input'><input type='text' maxlength='20' name='nation' /></td>
						<td></td>
					</tr>
				</table>
				</td>
		...
		{
			// Build SQL Query from nations db
			$query = "SELECT * FROM nations WHERE nation LIKE '%$trimmed%' ORDER BY nation";
			...
			if ($numrows == 0)
			{
				echo "<tr><td class='search_result_title' colspan='4'>Sorry, your search for the nation &quot;" . $trimmed . "&quot; returned zero results.</td></tr>";
			}
			else
			{
				...
				echo "<tr><th class='search_result_header' colspan='4' align='center'>You searched for the nation <strong>" . $var . "</strong>.</th></tr>";

				// begin to show results set
				echo "<tr><th class='search_result_header' width='20'>#</th><th class='search_result_header'>Nation</th><th class='search_result_header'>Nation Link</th><th class='search_result_header' width='100'>Private Message</th></tr>";
				$count = 1 + $s ;
				...
				while ($row = mysql_fetch_array($result))
				{
					...
					$result2 = mysql_query("SELECT * FROM users WHERE ID = '$ID'") or die(mysql_error());
					$ruler = mysql_fetch_array($result2) or die(mysql_error());
					$leader = stripslashes($ruler['username']);

					echo "<tr><td class='search_result_title'>" . $count . "</td><td class='search_result_info'><strong>" . $nation . "</strong><br />Ruled by " . $leader . "</td><td class='search_result_info'><a class='link_inline' href=\"nation.php?ID=$ID\">View " . $nation . "</a></td><td class='search_result_info'><a class='link_inline' href=\"pm_send.php?to=$leader\">Send a message</a></td></tr>" ;
					$count++ ;
				}
				...
				if ($s >= 1)
				{
					...
					$prevs = ($s - $limit);
					print "<a class='link_inline' href='$PHP_SELF?s=$prevs&nation=$var'>Prev 10</a>";
				}
				...
				if (!((($s + $limit) / $limit) == $pages) && $pages != 1)
				{
					...
					echo "<a class='link_inline' href='$PHP_SELF?s=$news&nation=$var'>Next 10</a>";
				}
